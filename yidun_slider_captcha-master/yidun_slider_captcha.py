...
def fake_drag(browser, knob, offset):
    ...
    :return:
    '''
    offsets, tracks = easing.get_tracks(offset, 5, 'ease_out_expo')    # 获取一种人性行为的滑动规则（总体参试一下几种滑动规则，还是ease_out_expo()该方法正确度比较高）
...
def slider_captcha(wait, driver):
    ...
    bg_img_obj = driver.find_element(By.XPATH,'//img[@class="yidun_bg-img"]')
    slider_img_obj = driver.find_element(By.XPATH,'//img[@class="yidun_jigsaw"]')
    ...
    distance = (distance * zoom)+15     # 这里是网易云网站的偏移量的设置方案    （根据实际出现验证码的背景图片大小的偏移量）
    #print ('偏移量：', distance)
...
def selenium_login(driver,recount):
  while(1):
   try:
    ...
    phonee = WebDriverWait(driver, 40).until(
        EC.presence_of_element_located((By.CSS_SELECTOR,
                                        'body > div.hwid-root > div > div:nth-child(1) > div.hwid-fullpage-container > div:nth-child(3) > span.hwid-fullpage-right-side > div.hwid-pwdlogin-root > span')))
    phonee.click()
    time.sleep(2)
    phonee = WebDriverWait(driver, 120).until(
        EC.presence_of_element_located((By.CSS_SELECTOR,
                                        'body > div.hwid-root > div > div:nth-child(1) > div.hwid-fullpage-container > div:nth-child(3) > span.hwid-fullpage-right-side > div.hwid-smslogin-root > div.hwid-widget-input.hwid-callingCodePhone-root.hwid-login-input-wrap > div > div.hwid-dropdown-root.countryCodeDropDown')))
    phonee.click()
    time.sleep(2)

    phonee = WebDriverWait(driver, 120).until(
        EC.presence_of_element_located((By.CSS_SELECTOR,
                                        'body > div.hwid-root > div > div:nth-child(1) > div.hwid-fullpage-container > div:nth-child(3) > span.hwid-fullpage-right-side > div.hwid-smslogin-root > div.hwid-widget-input.hwid-callingCodePhone-root.hwid-login-input-wrap > div > div.hwid-dropdown-root.countryCodeDropDown > div > div.hwid-vue-dropdown.hwid-default-theme > div > div.hwid-input-div > input')))
    ...
    phonee = WebDriverWait(driver, 120).until(
        EC.presence_of_element_located((By.CSS_SELECTOR,
                                        'body > div.hwid-root > div > div:nth-child(1) > div.hwid-fullpage-container > div:nth-child(3) > span.hwid-fullpage-right-side > div.hwid-smslogin-root > div.hwid-widget-input.hwid-callingCodePhone-root.hwid-login-input-wrap > div > div.hwid-input-root > input')))
    phonee.send_keys(fnumbers[recount])
    phonee = WebDriverWait(driver, 120).until(
        EC.presence_of_element_located((By.CSS_SELECTOR,
                                        'body > div.hwid-root > div > div:nth-child(1) > div.hwid-fullpage-container > div:nth-child(3) > span.hwid-fullpage-right-side > div.hwid-smslogin-root > div.hwid-widget-input.hwid-getAuthCode-input.hwid-login-input-wrap > div.hwid-input-area > div.hwid-getAuthCode')))
    phonee.click()
    ...
    while(1):
        try:
          if(tries%5==0):
              driver.quit()
              break
          slider_captcha(wait, driver)
          try:
              time.sleep(2)
              driver.find_element(By.CLASS_NAME, 'yidun_bg-img')
              tries+=1
          except:
              break
...
def main():
    ...
    # options.add_argument('--load-extension=C:\\Users\\Administrator\\Desktop\\Booking\\UltraSur')
    # options.add_extension('coordinates.crx')s
    while(1):
        try:
    ...
    while(1):
      try:
       driver.get('https://id7.cloud.huawei.com/CAS/portal/login.html')
       break
      except:
        pass
    time.sleep(2)
    while(1):
      selenium_login(driver=driver,recount=recount)
      time.sleep(10)
      count+=1
      print(count)
      driver.refresh()
